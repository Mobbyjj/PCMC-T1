...
model_dir_path = '/media/ssd/fanwen/T1mapping/Free_breathing/Model'
config = config_loader.update_config_with_args(config_loader.parse_args())
model_name = ['ISMRM', 'MICCAI_no_dice', 'MICCAI']
model_name = model_name[2]
# # ***************** preprocessing *****************
# just take the first 30 ones for testing
registered_test_set = np.zeros((30,160,160,11,5))
registered_seg_set = np.zeros((30,160,160,11,5))
...
if calc_flag:
    ...
    #    print(f"{k_fold = }")
        # ***************** Loading *****************
    data_set = data_preprocessing.load_test_set(config)
    ...
    for i,p in enumerate(data_set['test_ind']):
        for s in range(5):
            # here to examine one case by case (1st and 5th dim)
    ...
    save_dir_path = '/media/ssd/fanwen/T1mapping/Free_breathing/Model/' + model_name
    os.makedirs(save_dir_path, exist_ok=True)
    np.save(save_dir_path + '/registered_test_set_final30.npy',registered_test_set)
    np.save(save_dir_path + '/registered_seg_set_final30.npy',registered_seg_set)
